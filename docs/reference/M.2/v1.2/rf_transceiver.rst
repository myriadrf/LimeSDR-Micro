...
The `LMS7002M`_ analog baseband interface and control signals are described below.

Analog Baseband Interface
==========================

LMS7002M is using differential analog baseband IQ signals to transfer data to/from the NXP Baseband processor, rather than the digital LimeLight interface. The LMS7002M's internal DACs/ADCs and digital Transceiver Signal Processor (TSP) are bypassed, since the LA9310 baseband processor performs its own data conversion and digital signal processing.
...
.. note::
   Only one TX channel (TX1) is connected to BB processor; the second LMS7002M TX channel (TX2) baseband pins are connected to 15-pin FPC connector (X6) described in :doc:`BB_CON`.
...
* LMS_RXEN,receiver enable/disable signal is not controlled and is connected to VDIO_LMS (3.3V) power rail.
* LMS_TXEN – transmitter enable/disable signal connected to BB processor pin M15 via level converter (3.3V).
* LMS_RESET – LMS7002M reset is connected to I2C GPIO expander (IC14) GPA0 pin.
...
* LMS I2C Interface: can be used for LMS EEPROM content modification or debug purposes. The signals LMS_I2C_SCL and LMS_I2C_SDA are connected to EEPROM. They can be also connected to BB processors LA_I2C_SCL and LA_I2C_SDA.
...
.. table:: Table 2. LMS7002M RF transceiver signals
    ...
    +----------------------+----------------------------+-----------------------------+-----------------------+--------------------------+----------------------------------------------+
    | U29                  | TXEN                       | LMS_TXEN                    | M15                   | LNA1_EN/GPIO_11          | Via level converter (3.3V); pulled-up by R11 |
    +----------------------+----------------------------+-----------------------------+-----------------------+--------------------------+----------------------------------------------+
    | V34                  | RXEN                       | LMS_RXEN                    |                       |                          | Pulled-up by R12                             |
    +----------------------+----------------------------+-----------------------------+-----------------------+--------------------------+----------------------------------------------+
    | U33                  | CORE_LDO_EN                | LMS_CORE_LDO_EN             |                       |                          | Pulled-down by R15                           |
    +----------------------+----------------------------+-----------------------------+-----------------------+--------------------------+----------------------------------------------+
    | V30                  | LOGIC_RESET                |                             |                       |                          | GND                                          |
    +----------------------+----------------------------+-----------------------------+-----------------------+--------------------------+----------------------------------------------+
    | B22                  | tstao                      | LMS_TSTAO                   |                       |                          | Test pin connected to J2 via R230 (NF)       |
    +----------------------+----------------------------+-----------------------------+-----------------------+--------------------------+----------------------------------------------+
    | D22                  | tstdo<0>                   | LMS_TSTDO0                  |                       |                          | Test pin connected to J2 via R231 (NF)       |
    +----------------------+----------------------------+-----------------------------+-----------------------+--------------------------+----------------------------------------------+
    | F22                  | tstdo<1>                   | LMS_TSTDO1                  |                       |                          | Test pin connected to J2 via R232 (NF)       |
    +----------------------+----------------------------+-----------------------------+-----------------------+--------------------------+----------------------------------------------+